classdef AirSimClient < handle
    %AIRSIMCLIENT a Matlab client for HERCULES API. 
    ...
    methods(Static)        
        ...
        function carControls = getCarControls()
            carClient = py.hercules.CarClient();
            carClient.confirmConnection();
            carControls = py.hercules.CarControls();
        end

        function droneClient = getDroneControls()
            droneClient = py.hercules.MultirotorClient();
    ...
    methods
        ...
        function [image, timestamp] = getCameraImage(obj, sensorName, cameraType, vehicleName, annotationLayer)
            ...
            if cameraType == 1 || cameraType == 2 || cameraType == 3 || cameraType == 4
                image_request = py.hercules.ImageRequest(sensorName, int32(cameraType), true, false);
            elseif cameraType == 10
                image_request = py.hercules.ImageRequest(sensorName, int32(cameraType), false, false, annotationLayer);
            else
                image_request = py.hercules.ImageRequest(sensorName, int32(cameraType), false, false);
            end
            ...
            image_response = py.hercules.ImageResponse();
        ...
        function [images, timestamp] = getCameraImages(obj, sensorName, cameraTypes, vehicleName, annotationLayers)
            ...
            for i = 1: numel(cameraTypes)
                if cameraTypes(i) == 1 || cameraTypes(i) == 2 || cameraTypes(i) == 3 || cameraTypes(i) == 4
                    image_requests{i} = py.hercules.ImageRequest(sensorName, int32(cameraTypes(i)), true, false);
                elseif cameraTypes(i) == 10
                    image_requests{i} = py.hercules.ImageRequest(sensorName, int32(cameraTypes(i)), false, false, annotationLayers(i));
                else
                    image_requests{i} = py.hercules.ImageRequest(sensorName, int32(cameraTypes(i)), false, false);
                end
            end
        ...
        function [intrinsics, sensorPose] = getCameraInfo(obj, sensorName, vehicleName)
            ...
            tempRequest = py.hercules.ImageRequest(sensorName, int32(1), true, false);
            ...
            testImageResponse = py.hercules.ImageResponse();
