...
4. You should see a message saying the SITL PX4 app is waiting for the simulator (HERCULES) to connect.
You will also see information about which ports are configured for mavlink connection to the PX4 app.
The default ports have changed recently, so check them closely to make sure HERCULES settings are correct.
    ```
...
5. Now edit [HERCULES settings](settings.md) file to make sure you have matching UDP and TCP port settings:
    ```json
    {
        "SettingsVersion": 2.0,
        "SimMode": "Multirotor",
    ...
    information. The `Barometer` setting keeps PX4 happy because the default HERCULES barometer has a
...
7. Now run your Unreal HERCULES environment and it should connect to SITL PX4 via TCP. You should see
   a bunch of messages from the SITL PX4 window. Specifically, the following messages tell you that
   HERCULES is connected properly and GPS fusion is stable:
    ```
...
8. You should also be able to use QGroundControl with SITL mode.  Make sure there is no Pixhawk
   ...
   do not need to do this unless you plan to fly a drone manually in HERCULES.  Autonomous flight
...
You can simulate multiple drones in SITL mode using HERCULES. However, this requires setting up
